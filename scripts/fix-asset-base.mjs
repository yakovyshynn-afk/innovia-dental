...
// всередині самого коду (JS string-літерали в компонентах) — вони потрапляють
// у dist/assets/*.js як є.
//
...
//
// perf-задача 2026-09-15 (п.1.4) — self-host шрифтів через @font-face
// url("/fonts/...") у src/index.css НЕ потребує цього патчу: Vite сам коректно
// переписує кореневі url()-посилання в CSS і href у <link> в index.html з base-
// префіксом на етапі власного білду (перевірено: dist/*.css і dist/index.html
// вже містять /innovia-dental/fonts/... без стороннього втручання). "/images/..."
// патчиться тут окремо саме тому, що це рядки всередині JS (src=... в JSX), а не
// url()/href — Vite їх не розпізнає як ассет-посилання і не чіпає.
//
